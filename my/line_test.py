...
from line_linecontrol import LineControl
...
def main():
    try:
        ...
        print("机器人已进入站立状态，开始黄线居中调整（基于图像底部区域检测）")
...
class Robot_Ctrl(object):
    def __init__(self):
        ...
        # 黄线检测参数
        self.yellow_lower = np.array([10, 80, 80])  # 放宽HSV下限，适应更多黄线场景
        self.yellow_upper = np.array([45, 255, 255])  # 放宽HSV上限
        self.min_contour_area = 50  # 降低最小轮廓面积阈值
        self.roi_ratio = 0.8  # ROI区域比例（保留图像下方80%）
        
        # 目标区域参数（底部30%-40%）
        self.bottom_start_ratio = 0.6  # 底部30%区域开始（60%高度）
        self.bottom_end_ratio = 0.7    # 底部40%区域结束（70%高度）

        self.min_valid_points = 2  # 降低有效点数量要求

        # 控制参数
        self.adjustment_distance = 0.05  # 每次调整距离(米)
        self.adjustment_vel = 0.03  # 调整速度(米/秒)
    ...
    def run(self):
        try:
            ...
            if self.debug_mode:
                print("[系统] 初始化完成，开始基于图像底部30%-40%区域的黄线监测")
        except Exception as e:
            print(f"运行初始化错误: {e}")
            raise
    ...
    def detect_line_features(self, image, is_left_cam, image_ts):
        ...
        try:
            ...
            if self.debug_mode:
                ...
                # 标记目标区域
                area_start = int(h * self.bottom_start_ratio)
                area_end = int(h * self.bottom_end_ratio)
                cv2.line(debug_img, (0, area_start), (w, area_start), (0, 165, 255), 2)  # 橙色线标记30%边界
                cv2.line(debug_img, (0, area_end), (w, area_end), (0, 165, 255), 2)     # 橙色线标记40%边界
                
                # 标记最近点
                cv2.circle(debug_img, (int(closest_pixel[0]), int(closest_pixel[1])), 5, (0, 255, 255), -1)
                
                # 判断是否在目标区域
                is_in_area = area_start <= closest_pixel[1] <= area_end
                status_text = f"底部30%-40%: {'是' if is_in_area else '否'}"
                cv2.putText(debug_img, status_text, (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 255), 2)
                
                # 添加更多调试信息
                debug_info = [
                    f"检测区域: 底部{int(self.bottom_start_ratio*100)}%-{int(self.bottom_end_ratio*100)}%",
                    f"当前高度: {int(closest_pixel[1]/h*100)}%",
                    f"在目标区域: {'是' if is_in_area else '否'}"
                ]
                for i, text in enumerate(debug_info):
                    cv2.putText(debug_img, text, (10, 60 + i*30), 
                               cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 2)
                
                cv2.imshow(f"{cam_type}相机黄线检测(30%-40%区域)", debug_img)
                cv2.waitKey(1)
    ...
    def is_in_target_area(self, line_data):
        """判断黄线是否出现在图像底部30%-40%区域"""
        if not line_data:
            return False
        ...
        # 计算目标区域边界
        area_start = int(h * self.bottom_start_ratio)  # 底部30%开始
        area_end = int(h * self.bottom_end_ratio)     # 底部40%结束

        return area_start <= v <= area_end

    def check_line_position(self):
        """检查黄线位置（基于底部30%-40%区域判断）"""
        ...
        left_in_area = self.is_in_target_area(self.left_line_data)
        right_in_area = self.is_in_target_area(self.right_line_data)

        print(f"左黄线底部30%-40%: {left_in_area}, 右黄线底部30%-40%: {right_in_area}")

        if left_in_area and not right_in_area:
            print("左黄线出现在目标区域，需要向右调整")
            return True, "right"
        elif right_in_area and not left_in_area:
            print("右黄线出现在目标区域，需要向左调整")
            return True, "left"
        elif left_in_area and right_in_area:
            print("左右黄线均出现在目标区域，无法安全调整")
            return False, ""
        else:
            return False, ""
    ...
    def perform_line_detection_and_adjustment(self, msg):
        ...
        while adjustment_count < max_adjustments:
            ...
            # 显式计算最新状态（确保每次更新）
            left_in_area = self.is_in_target_area(self.left_line_data) if self.left_line_data else False
            right_in_area = self.is_in_target_area(self.right_line_data) if self.right_line_data else False

            # 打印当前状态
            print(f"\n调整次数：{adjustment_count + 1}/{max_adjustments}")
            print(f"当前状态 - 左黄线底部30%-40%: {left_in_area}, 右黄线底部30%-40%: {right_in_area}")
            ...
            # 检查终止条件
            if (not left_in_area and not right_in_area) or (left_in_area and right_in_area):
                print("\n满足终止条件，停止调整")
                break

            # 检查停滞状态
            if left_in_area == prev_left_state and right_in_area == prev_right_state:
                stall_count += 1
                if stall_count >= 2:
                    print("\n检测到调整停滞，停止调整")
                    break
                ...
                stall_count = 0
                prev_left_state = left_in_area
                prev_right_state = right_in_area
            ...
            if adjustment_needed:
                print(f"检测到黄线出现在目标区域，向{direction}方向调整")
        ...
        print(f"\n连续调整结束，最终状态 - 左黄线底部30%-40%: {left_in_area}, 右黄线底部30%-40%: {right_in_area}")
