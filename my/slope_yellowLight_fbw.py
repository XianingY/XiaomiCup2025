...
class YellowLightDetector(Node):
    ...
    def detect_yellow_light_in_upper_region(self, rgb_image, region_percent=0.3):
        """检测图像上侧指定百分比区域中的黄色区域"""
        # 如果检测未启用，直接返回空结果
    ...
    def image_callback(self, rgb_msg, depth_msg):
        ...
        try:
            ...
            # 检测黄色区域（仅上侧20%区域）
            yellow_regions = self.detect_yellow_light_in_upper_region(rgb_image, 0.3)
...
def robot_control_thread(stop_event, detector, announcer):
    ...
    try:
        ...
        # 检查站立期间是否检测到黄灯
        yellow_light_detected = detector.get_yellow_light_detected()
        # detector.set_detection_enabled(False)  # 暂时关闭检测

        if yellow_light_detected:
            print("站立期间检测到黄灯，开始直行并等待距离小于1.2米")
            # 如果检测到黄灯，开始直行并等待距离触发
            detector.set_detection_enabled(True)  # 重新启用检测
            ...
            # 持续直行，直到检测到黄灯距离小于1.2米
            while rclpy.ok() and not has_stopped:
                # 检查是否需要停止
                if stop_event.is_set():
                    ...
                    # 确保不会出现负数睡眠
                    continue_time = max(0, 25 - walking_duration)
        ...
        Ctrl.quit()
        try:
            rclpy.shutdown()  # 让 executor.spin() 退出
        except Exception as e:
