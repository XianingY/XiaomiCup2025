...
class YellowLightDetector(Node):
    ...
    def detect_yellow_light(self, rgb_image):
        """检测图像上侧40%区域中的黄色区域（不限制为圆形）"""
        # 获取图像尺寸并计算上侧40%区域
        height, width = rgb_image.shape[:2]
        upper_region_height = int(height * 0.4)
        upper_region = rgb_image[0:upper_region_height, :]  # 只取上侧40%区域
        ...
        for contour in contours:
            ...
            # 获取轮廓的最小外接圆（仅用于获取中心和半径，不做圆形判断）
            (x, y), radius = cv2.minEnclosingCircle(contour)
            
            # 将坐标转换回原始图像尺寸（考虑缩放和上侧区域偏移）
            center = (int(x / scale), int(y / scale))  # 上侧区域内的坐标
            # 转换为原始图像的全局坐标（加上上侧区域的偏移）
            original_center = (center[0], center[1])  # y坐标已在0~upper_region_height范围内
        ...
        cv2.putText(rgb_image, "检测区域", (10, int(upper_region_height/2)), 
    ...
    def image_callback(self, rgb_msg, depth_msg):
        ...
        try:
            ...
            # 检测黄色区域（仅上侧40%）
            yellow_regions = self.detect_yellow_light(rgb_image)
            min_detected_distance = None
    ...
    def display_loop(self):
        ...
        while rclpy.ok():
            with self.image_lock:
                if self.latest_image is not None:
                    cv2.imshow("黄色区域检测与距离显示", self.latest_image)
       ...
       # detector.set_detection_enabled(True)
        ...
        while rclpy.ok():
            ...
            if stop_event.is_set() and not has_stopped:
                ...
                # 确保不会出现负数睡眠
                continue_time = max(0, 30 - walking_duration)
