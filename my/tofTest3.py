    ...
    def obstacle_callback(self, msg):
        try:
            ...
            with self.parent.left_count_lock:
                if data == "left":
                    self.parent.left_count += 1
                    self.parent.right_count = 0  # 重置right计数
                    if self.parent.left_count % 5 == 0:  # 每5次打印一次进度
                        self.get_logger().info(f"已连续收到{self.parent.left_count}次'left'")

                    # 检查是否达到20次
                    if self.parent.left_count >= 20:
                        self.detection_result = "left"
                        self.get_logger().info("检测到连续20次'left'，返回结果")

                elif data == "right":
                    self.parent.right_count += 1
                    self.parent.left_count = 0  # 重置left计数
                    if self.parent.right_count % 5 == 0:  # 每5次打印一次进度
                        self.get_logger().info(f"已连续收到{self.parent.right_count}次'right'")

                    # 检查是否达到20次
                    if self.parent.right_count >= 20:
                        self.detection_result = "right"
                        self.get_logger().info("检测到连续20次'right'，返回结果")
                else:
                    # 如果收到非"left"或"right"数据，重置计数器
                    self.parent.left_count = 0
                    self.parent.right_count = 0
        except Exception as e:
            self.get_logger().error(f"处理障碍物数据时出错: {str(e)}")
...
class EnhancedRobotCtrl(Robot_Ctrl):
    ...
    def wait_for_continuous_left(self, required_count=20, timeout=120):
        """等待连续收到指定次数的"left"或"right"数据"""
        print(f"等待连续{required_count}次收到'left'或'right'数据...")
        ...
        while True:
            ...
            if current_left_count >= required_count:
                print(f"成功连续收到{required_count}次'left'数据")
                # 重置计数器，为下一次检测做准备
                with self.left_count_lock:
                    self.left_count = 0
                return (True, "left")  # 返回成功标志和方向

            if current_right_count >= required_count:
                print(f"成功连续收到{required_count}次'right'数据")
                # 重置计数器，为下一次检测做准备
                with self.left_count_lock:
                    self.right_count = 0
                return (True, "right")  # 返回成功标志和方向

            if time.time() - start_time > timeout:
                print(f"超时: 在{timeout}秒内未连续收到{required_count}次'left'或'right'数据")
                return False, None
...
class RobotQRRunner():
    ...
    # A处的行为
    def qrReadA(self,defult_code="A-1"):
        Ctrl = EnhancedRobotCtrl()
        Ctrl.run()
        try:
            print("=== 开始执行qrReadA流程 ===")
            ...
            #第一次右转 - 使用新方法
            print("3. 第一次右转到90度")
            Ctrl.send_move_command(11, 27, [0, 0, -0.5],duration=4000,step_height=[0.06, 0.06])
            print("第一次右转完成，准备继续")
            Ctrl.Wait_finish(11,27)
            time.sleep(4.1)
            ...
            while True:
                print("111")
                try:
                    qr_data = Ctrl.qr_queue.get(timeout=10)  # 最多等 10 秒
                    print(f"[主线程] 收到二维码: {qr_data}")
                except queue.Empty:
                    print("[超时] 未识别到二维码，执行默认策略")
                    qr_data = defult_code  # 默认值（你可以改成"A-2"或者"B-1"等）

                if qr_data == "A-1":
                    Ctrl.announcer.speak(50003)
                    direction = self.do_action_a1(Ctrl)
                    break
                elif qr_data == "A-2":
                    Ctrl.announcer.speak(50004)
                    direction = self.do_action_a2(Ctrl)
                    break
                else:
                    print(f"未知二维码: {qr_data}")
                    count = count + 1
                    if count >= 2:
                        Ctrl.announcer.speak("A区库位1")
                        direction = self.do_action_a1(Ctrl)
                        break
                    continue
        ...
        return  qr_data,direction

    # B处的行为
    def qrReadB(self, direction,defult_code="B-1"):
        # 使用增强的机器人控制类
        Ctrl = EnhancedRobotCtrl()
        Ctrl.run()
        msg = robot_control_cmd_lcmt()
        ...
        try:
            ...
            while True:
                ...
                try:
                    ...
                    print("[超时] 未识别到二维码，执行默认策略 B-1")
                    qr_data = defult_code  # 默认值（你可以改成"A-2"或者"B-1"等）

                if qr_data == "B-1":
                    Ctrl.announcer.speak(50005)
                    self.do_action_b1(Ctrl, direction)
                    break
                elif qr_data == "B-2":
                    Ctrl.announcer.speak(50006)
    ...
    def qrReadBack(self,aCode):
        ...
        try:
            ...
            #二维码是A-2，返回时在A1卸货
            if aCode == "A-2" :
                direction = self.do_action_a1_return(Ctrl)
            else :
                #A区二维码是A-1，卸货去A-2
                direction = self.do_action_a2_return(Ctrl)
    ...
    def do_action_a1(self, Ctrl):
        ...
        Ctrl.send_move_command(11, 27, [0, -0.5,0],duration=4000,step_height=[0.06, 0.06])
        #Ctrl.Wait_finish(11, 27)
        time.sleep(4)
        ...
        #     print("未能检测到足够的信号，继续执行但可能存在风险")

        success, direction = Ctrl.wait_for_continuous_left(20)
        if not success:
            print("未能检测到足够的信号，继续执行但可能存在风险")
        else:
            print(f"成功检测到连续20次'{direction}'信号")
    ...
    def do_action_a2(self, Ctrl):
        ...
        print("再左移")
        # Ctrl.turn_to_direction(180)
        Ctrl.send_move_command(11, 27, [0, 0.5, 0],duration=4100,step_height=[0.06, 0.06])
        time.sleep(4)
    ...
    def do_action_b1(self, Ctrl, direction):
        ...
        success, tof_direction = Ctrl.wait_for_continuous_left(20)
        if not success:
            print("未能检测到足够的信号，继续执行但可能存在风险")
        else:
            print(f"成功检测到连续20次'{tof_direction}'信号")
    ...
    def do_action_a1_return(self, Ctrl):
        print("进入识别完成后续操作")
        # 先直行
        Ctrl.send_move_command(
            mode=11,  # Locomotion
    ...
    def do_action_a2_return(self, Ctrl):
        print("进入识别完成后续操作")
        # 先直行
        Ctrl.send_move_command(
            mode=11,  # Locomotion
    ...
    def arrow_Read(self,direction):
        Ctrl = EnhancedRobotCtrl()
        Ctrl.run()
        Ctrl.send_move_command(mode=12, gait_id=0, vel_des=[0, 0, 0], duration=0)
        Ctrl.Wait_finish(12, 0)
        if direction == "right":
            Ctrl.announcer.speak(50002)
        elif direction == "left":
            Ctrl.announcer.speak(50001)
        ...
        try:
            ...
            Ctrl.send_move_command(mode=11, gait_id=27, vel_des=[0.3, 0, 0], duration=1900,
                                        step_height=[0.06, 0.06])
            Ctrl.Wait_finish(11, 27)
            ...
            if direction == "right":
                Ctrl.send_move_command(mode=11, gait_id=27, vel_des=[0, -0.5, 0], duration=5000,
                                        step_height=[0.06, 0.06])
                time.sleep(5.0)
                Ctrl.send_move_command(mode=11, gait_id=27, vel_des=[0, 0, 0.5], duration=0,
                                        step_height=[0.06, 0.06])
                time.sleep(7.3)
            elif direction == "left":
                Ctrl.send_move_command(mode=11, gait_id=27, vel_des=[0, 0.5, 0], duration=3000,
                                        step_height=[0.06, 0.06])
                time.sleep(3.0)
